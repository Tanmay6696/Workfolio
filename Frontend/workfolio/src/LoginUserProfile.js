  ...
  const LoginUserProfile = () => {
    const [userdata, setUserdata] = useState(null); // Initialize as an empty array
    ...
    const getdata = async () => {
      try {
        ...
        console.log("response from getUserdata: d d d d d d", userdata);
              ...
              <div>
                ...
                <ul>
                  {user.skills.map((skillId) => (
                    <li>{userdata[skillId].SkillName?userdata[skillId].SkillName:""}</li>
                  ))}
                </ul>
              </div>
              <div>
